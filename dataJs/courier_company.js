...
$(document).ready(function() {
    $("#save_data").submit(function(event) {
        ...
        var address_cou = $('#address_cou').val();
        var phone_cou = $('#phone_cou').val(); 
        var country_cou = $('#country_cou').val();
        var city_cou = $('#city_cou').val();
        var postal_cou = $('#postal_cou').val();
       ...
       // Almacena los nombres de los campos faltantes
        ...
        if (!name_com) missingFields.push(message_error_form50);
        if (!address_cou) missingFields.push(message_error_form51);
        if (!phone_cou) missingFields.push(message_error_form52);
        if (!country_cou) missingFields.push(message_error_form53);
        if (!city_cou) missingFields.push(message_error_form54);
        if (!postal_cou) missingFields.push(message_error_form55);
            ...
            success: function(response) {
                ...
                if (response.status === 'success') {
                    Swal.fire({
                        ...
                        window.location.href = 'courier_company_list.php';
                    });
                } else {
                    Swal.fire({
                        type: 'error',
