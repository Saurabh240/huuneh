...
function cdp_load(page) {
  ...
  $("#loader").fadeIn("slow");
  $.ajax({
    url: "ajax/tools/templates_whatsapp/templates_whatsapp_ajax.php",
    data: parametros,
...
$(document).ready(function() {
    $("#save_data").submit(function(event) {
       ...
       // Almacena los nombres de los campos faltantes
        ...
        if (!title) missingFields.push(message_error_form23);
        if (!description) missingFields.push(message_error_form24);
        if (!body) missingFields.push(message_error_form25);
        ...
        $.ajax({
            url: "./ajax/tools/templates_whatsapp/templates_whatsapp_add_ajax.php",
            type: 'POST',
            data: data,
            contentType: false,
            cache: false,
            ...
            success: function(response) {
                ...
                if (response.status === 'success') {
                    Swal.fire({
                        ...
                        window.location.href = 'templates_whatsapp.php';
                    });
                } else {
                    Swal.fire({
                        type: 'error',
...
$(document).ready(function() {
    ...
    $("#update_data").submit(function(event) {
        ...
        // Realizar la solicitud AJAX
        $.ajax({
            url: "./ajax/tools/templates_whatsapp/templates_whatsapp_edit_ajax.php",
            type: 'POST',
            data: data,
            contentType: false,
            cache: false,
    ...
    swal({
        ...
        preConfirm: function() {
            return new Promise(function(resolve) {
                $.ajax({
                        url: './ajax/tools/templates_whatsapp/templates_whatsapp_delete_ajax.php',
                        type: 'POST',
                        data: {
                            'id': id,
                        },
