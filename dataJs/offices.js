...
$(document).ready(function() {
    $("#save_data").submit(function(event) {
        ...
        var name_off = $('#name_off').val(); 
        var code_off = $('#code_off').val();
        var address = $('#address').val(); 
        var city = $('#city').val();
        var phone_off = $('#phone_off').val();
       ...
       // Almacena los nombres de los campos faltantes
        ...
        if (!name_off) missingFields.push(message_error_form41);
        if (!code_off) missingFields.push(message_error_form42);
        if (!address) missingFields.push(message_error_form43);
        if (!city) missingFields.push(message_error_form44);
        if (!phone_off) missingFields.push(message_error_form45);
            ...
            success: function(response) {
                ...
                if (response.status === 'success') {
                    Swal.fire({
                        ...
                        window.location.href = 'offices_list.php';
                    });
                } else {
                    Swal.fire({
                        type: 'error',
