...
$(document).ready(function() {
    $("#save_data").submit(function(event) {
       ...
       // Almacena los nombres de los campos faltantes
        ...
        if (!name_branch) missingFields.push(message_error_form46);
        if (!branch_address) missingFields.push(message_error_form47);
        if (!branch_city) missingFields.push(message_error_form48);
        if (!phone_branch) missingFields.push(message_error_form49);
            ...
            success: function(response) {
                ...
                if (response.status === 'success') {
                    Swal.fire({
                        ...
                        window.location.href = 'branchoffices_list.php';
                    });
                } else {
                    Swal.fire({
                        type: 'error',
